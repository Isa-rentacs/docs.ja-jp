...
```  
 ...
 この機能は、`SaveStateAttribute` クラスに実装されています。 実装`IOperationBehavior`操作ごとに、WCF ランタイムを変更するクラス。 この属性でマークされたメソッドは、WCF ランタイムによって呼び出される、`ApplyBehavior`中に、適切なメソッド`DispatchOperation`が構築されます。 このメソッドの実装のコードは 1 行です。  
